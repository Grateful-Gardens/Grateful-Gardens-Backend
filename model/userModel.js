...
class Users {
  ...
  static async getAllFriendsFromDB(user_id) {
    const sql = `SELECT users.*, friend_two FROM friendships JOIN users ON friendships.friend_two = users.user_id WHERE friend_one = ($1)`;
    const dbResult = await pool.query(sql, [user_id]);
    return dbResult.rows;
  }

//   static async getAllFriendsFromDBTwo(user_id) {
//     const sql = `SELECT users.*, friend_two FROM friendships JOIN users ON friendships.friend_one = users.user_id WHERE friend_two = ($1)`;
//     const dbResult = await pool.query(sql, [user_id]);
//     return dbResult.rows;
//   }
  ...
  static async unFriendFromDB(user_id, friend_two) {
    if (!friend_two) {
        throw new Error(`FRIENDSHIP WITH ID: ${friend_two} DOES NOT EXIST`);
    }
